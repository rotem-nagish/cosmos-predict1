    ...
    def __init__(
        ...
        **kwargs,  # absorb unused args from config system
        ...
        Args:
            ...
            pose_normalization (str): Pose normalization strategy
            limit (int): Optional limit on number of videos
        """
            ...
            try:
                ...
                try:
                    ...
                    with open(pose_path, "rb") as f:
                        ...
                        data["gt_pose"].body.data = data["gt_pose"].body.data[start_frame:start_frame+self.sequence_length, :, :]
                        data["gt_pose"].body.confidence = data["gt_pose"].body.confidence[start_frame:start_frame+self.sequence_length, :, :]
                except Exception:
                    data["gt_pose"] = []
                ...
                data["loss_mask"] = torch.ones_like(video, dtype=torch.float32)  # [C, T, H, W]
                mask_path = video_path.replace("/processed/", "/pose_masks/")[:-len(".mp4")]+".pt"
                if os.path.exists(mask_path):
                    data["loss_mask"] = torch.load(mask_path)[start_frame:start_frame+self.sequence_length]
